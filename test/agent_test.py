...
async def run_agent():
    ...
    autonomous_system_prompt = (
        "You are a highly autonomous assistant skilled in PDF processing and task delegation. You can load PDFs, query their content, generate new documents, and manage sub-agents for complex tasks. "
        ...
        "7.  **Create New PDF:** Use the `create_document_from_description` tool. For the `requested_filename` argument, derive a name from the original, like `originalFilename_filled_autonomously` (e.g., if original was `test.pdf`, use `test_filled_autonomously`). Note the exact path of the newly created PDF.\n"
        "8.  **Verify Generated PDF:** Load the newly created PDF using `load_pdf_document` with a unique verification ID (e.g., 'generated_doc_verification'). Then, use `query_pdf_document` on this verification ID with a detailed query to check if *all* the intended substitutions (based on the analysis in step 4 and substitutions in step 5) are present and correctly formatted in the generated PDF. The query should be specific, e.g., 'Verify if the following information was correctly inserted: [list the specific substituted data points]. Report any missing or incorrect information.' Analyze the query result to assess if the writing model performed as intended and if any further changes are needed.\n"
        "9.  **Report Outcome:** Your final response to the user must confirm task completion. State the path to the NEWLY created PDF. Briefly mention that placeholders were identified (or an attempt was made) and filled autonomously using generic data. Crucially, report the outcome of the detailed PDF verification step, noting if all intended changes were found or if any were missing/incorrect. Do NOT ask for user confirmation at any step; execute the entire process autonomously.\n\n"
